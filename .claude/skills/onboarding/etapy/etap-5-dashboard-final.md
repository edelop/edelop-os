...
Po powtórce powiedz mniej więcej: "To ostatni etap. Dziś domykamy system: dostaniesz
...
- Zapowiedz: "Teraz trzeci workflow, którego jeszcze nie widziałeś(-aś) w akcji: Lint,
...
- Zaktualizuj `.onboarding/postep.md`: Etap 5 na `ukonczony` z dzisiejszą datą - wszystkie
...
- CELEBRACJA. Policz konkretne liczby z systemu (strony w `projekty/`, `obszary/`, `cele/`,
  ...
  [K] wpisów w dzienniku, dashboard i mapę połączeń. Tydzień temu to był pusty folder.
  I najważniejsze: od dziś nie ma już lekcji. System działa bez skilla - otwierasz folder,
  wpisujesz `claude` i po prostu mówisz, czego potrzebujesz."

### 7. Co dalej

Krótko, bez przytłaczania - trzy rzeczy i jedna furtka:
...
- [ ] `.onboarding/postep.md`: wszystkie etapy `ukonczony` lub `pominiety`
- [ ] Wpis `## [YYYY-MM-DD] onboarding | ukonczony` na końcu `log.md`
- [ ] Obsidian jest opcjonalny - jego brak NIE blokuje ukończenia etapu

## Praca domowa

Nie ma. Skończyły się prace domowe - zaczęło się życie z systemem. Powiedz mniej więcej:
"Zamiast pracy domowej masz jutro zwykły dzień: zacznij go od pytania Co mam dziś do
zrobienia? i pozwól systemowi pracować. To wszystko."
...
- W `.onboarding/postep.md` ustaw Etap 5 na `ukonczony` z dzisiejszą datą. Upewnij się,
  że w tabeli nie został żaden status `do-zrobienia` ani `w-trakcie`.
- Sekcja "Gdzie skończyliśmy": wpisz "Onboarding ukończony w całości [YYYY-MM-DD]. System
  działa samodzielnie - schemat w CLAUDE.md, ściąga w INSTRUKCJA.md."
- Sekcja "Praca domowa": wpisz "(brak - onboarding zakończony)".
